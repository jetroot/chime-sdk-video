...
module.exports = async function (fastify, opts) {
  fastify.get(
    ...
    async function (request, reply) {
      ...
      // Define the parameters for the getMeeting operation
      
      let foundMeeting = false;
      try {
        let meeting = await chime
            .getMeeting({ MeetingId: request.query.room })
            .promise();

        if (meeting.$response.httpResponse.statusCode === 200) {
            foundMeeting = true;
        } else {
            foundMeeting = false;
        }
      ...
      // if (isExists.$response.httpResponse.statusCode === 200) {
      //   foundMeeting = true
      // } else {
      //   foundMeeting = false;
      // }
      // console.log('=================foundMeeting========================== ', response);
